...
import time
import gpuTool
from sendGpuInfo import updateGpuInfo, updateDone
...
def getPowerDrawPerSecond(shared_isStart, shared_powerDraw_1epoch):
    ...
    while True:
        if (shared_isStart.value):
            sumPowerDraw = gpuTool.getGpuPowerDraw()
            shared_powerDraw_1epoch.value += float(sumPowerDraw)
            time.sleep(1)


def train_cifar10_with_gpu(shared_isStart, shared_powerDraw_1epoch, shared_fepochd, shared_fepochPer, shared_fepochPow, shared_epoch, shared_epochPow, shared_isEpochDone, shared_isLearningDone, shared_epochPer):
    # 데이터셋 불러오기 및 전처리
    ...
    num_epochs = 5
    shared_epoch.value = num_epochs #서버로 보낼 에폭
    ...
    for epoch in range(num_epochs):
        ...
        #print(f"isStart : {shared_isStart.value}")
        shared_isStart.value = True # 파워소모측정 시작
        #print(f"isStart : {shared_isStart.value}")
        ...
        shared_isStart.value = False # 파워소모 측정 종료
        ...
        # 처음 에폭의 결과를 전달
        if epoch == 0:
            shared_fepochd.value = True
            shared_fepochPer.value = performance
            shared_fepochPow.value = shared_powerDraw_1epoch.value
            # 첫 에폭이 끝났다는 것과 성능과 소모전력 전달
        ...
        #서버에 전송할 전력소모 변수 업데이트
        shared_epochPow.value = shared_powerDraw_1epoch.value
        shared_epochPer.value = performance
        shared_isEpochDone.value = True
...
if __name__ == "__main__":
    ...
    shared_fepochd = manager.Value('b', False)
    shared_powerDraw_1epoch = manager.Value('d', 0.0)
    shared_fepochPer = manager.Value('d', 0.0)
    shared_fepochPow = manager.Value('d', 0.0)
    shared_epoch = manager.Value('i', 0)
    shared_epochPow = manager.Value('d', 0.0)
    shared_epochPer = manager.Value('d', 0.0)
    shared_isEpochDone = manager.Value('b', False)
    shared_isLearningDone = manager.Value('b', False)



    profiler_process = multiprocessing.Process(target = updateGpuInfo, args=(shared_fepochd, shared_fepochPer, shared_fepochPow, shared_epoch, shared_epochPow, shared_isEpochDone, shared_isLearningDone, shared_epochPer))
    profiler_process2 = multiprocessing.Process(target=getPowerDrawPerSecond, args=(shared_isStart, shared_powerDraw_1epoch))
    profiler_process3 = multiprocessing.Process(target=train_cifar10_with_gpu, args=(shared_isStart, shared_powerDraw_1epoch, shared_fepochd, shared_fepochPer, shared_fepochPow, shared_epoch, shared_epochPow, shared_isEpochDone, shared_isLearningDone, shared_epochPer))
