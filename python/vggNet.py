...
def train_model(shared_isStart, shared_powerDraw_1epoch, shared_fepochd, shared_fepochPer, shared_fepochPow, shared_epoch,
    shared_epochPow, shared_isEpochDone, shared_isLearningDone, shared_epochPer):
    ...
    # 모델 학습
    for epoch in range(epoch):  # 데이터셋을 여러번 반복
...
if __name__ == "__main__":
    ...
    profiler_process = multiprocessing.Process(target=updateGpuInfo, args=(
    shared_fepochd, shared_fepochPer, shared_fepochPow, shared_epoch, shared_epochPow, shared_isEpochDone,
    shared_isLearningDone, shared_epochPer))
    profiler_process2 = multiprocessing.Process(target=getPowerDrawPerSecond,
                                                args=(shared_isStart, shared_powerDraw_1epoch))
    profiler_process3 = multiprocessing.Process(target=train_model, args=(
    shared_isStart, shared_powerDraw_1epoch, shared_fepochd, shared_fepochPer, shared_fepochPow, shared_epoch,
    shared_epochPow, shared_isEpochDone, shared_isLearningDone, shared_epochPer))
